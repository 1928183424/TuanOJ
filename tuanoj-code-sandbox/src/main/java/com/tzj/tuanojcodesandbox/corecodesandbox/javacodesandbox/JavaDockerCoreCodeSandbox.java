...
/**
 * java 核心代码模式 Docker 代码沙箱
 */
@Component
public class JavaDockerCoreCodeSandbox extends JavaCoreCodeSandboxTemplate {
    ...
    // Docker镜像名称常量
    private static final String DOCKER_IMAGE = "openjdk:8-alpine";
...
//            initDockerImage(dockerClient);
            ...
            // 执行代码并收集结果
            executeJavaCodeInContainer(dockerClient, containerId, inputList, executeMessageList);
    ...
    private void initDockerImage(DockerClient dockerClient) {
        ...
        if (FIRST_INIT) {
            synchronized (JavaDockerCoreCodeSandbox.class) {
                if (FIRST_INIT) {
     ...
     * 在容器中执行Java代码
     */
    private void executeJavaCodeInContainer(
        ...
        for (String inputArgs : inputList) {
            ...
            String[] cmdArray = ArrayUtil.append(new String[]{"java", "-cp", "/app", "Main"}, inputArgs);
