...
/**
 * cpp 核心代码模式 Docker 代码沙箱
 */
@Component
public class CppDockerCoreCodeSandbox extends CppCoreCodeSandboxTemplate {
    ...
    // 内存限制常量
    private static final long MEMORY_LIMIT = 200 * 1000 * 1000L;
...
//            initDockerImage(dockerClient);
            ...
            // 执行代码并收集结果
            executeCppCodeInContainer(dockerClient, containerId, inputList, executeMessageList);
    ...
    private void initDockerImage(DockerClient dockerClient) {
        ...
        if (FIRST_INIT) {
            synchronized (CppDockerCoreCodeSandbox.class) {
                if (FIRST_INIT) {
    ...
    private String createAndStartContainer(DockerClient dockerClient, String userCodeParentPath) {
        ...
        CreateContainerResponse container = dockerClient.createContainerCmd(DOCKER_IMAGE)
                ...
                .withReadonlyRootfs(false)       // C++可执行文件通常需要执行权限，所以容器不能是完全只读的。
     ...
     * 在容器中执行Cpp代码
     */
    private void executeCppCodeInContainer(
        ...
        // 首先给可执行文件添加执行权限
        try {
            String[] chmodCmd = {"/bin/chmod", "+x", "/app/main"};
            ExecCreateCmdResponse chmodExecResponse = dockerClient.execCreateCmd(containerId)
                    .withCmd(chmodCmd)
                    .withAttachStderr(true)
                    .withAttachStdout(true)
                    .exec();

            dockerClient.execStartCmd(chmodExecResponse.getId())
                    .exec(new ExecStartResultCallback())
                    .awaitCompletion(10, TimeUnit.SECONDS);

            System.out.println("已添加执行权限");
        } catch (Exception e) {
            System.err.println("添加执行权限失败: " + e.getMessage());
        }
        ...
        for (String inputArgs : inputList) {
            ...
            String[] cmdArray = ArrayUtil.append(new String[]{"/app/main"}, inputArgs);
